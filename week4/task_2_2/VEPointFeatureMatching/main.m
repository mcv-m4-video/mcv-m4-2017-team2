...
function main
    ...
    video_images_dir = 'traffic';  % 'traffic', 'seq45' or 'seq157';
    video_images_format = '*.jpg';  % '*.jpg' for traffic, '*.png' for seq45 and seq157
    video_name = strcat(video_images_dir, '.avi');

    % 1. As the example works with avi videos, we transform our images sequence into a video to use their code directly.
    if exist(strcat(working_dir, '/', video_images_dir, '.avi'), 'file') == 0
        createVideoFromImagesSequence(working_dir, video_images_dir, video_images_format, video_name);
    end
    
    % 2. Read video into var
    filename = strcat(working_dir, '/', video_name);
    hVideoSrc = vision.VideoFileReader(filename, 'ImageColorSpace', 'Intensity');
    

    % 3.1. Show entire procedure applied to first two frames
    %PointFeatureMatchingVideoStabilizationProcedure(hVideoSrc);

    % 3.2. Apply video stabilization to video and show results
    PointFeatureMatchingVideoStabilization(hVideoSrc);
    
end


function PointFeatureMatchingVideoStabilization(hVideoSrc)
...
function PointFeatureMatchingVideoStabilizationProcedure(hVideoSrc)
    ...
    % Our goal is to determine a transformation that will correct for the distortion between the two frames. We can use the estimateGeometricTransform function for this, which will return an affine transform. As input we must provide this function with a set of point correspondences between the two frames. To generate these correspondences, we first collect points of interest from both frames, then select likely correspondences between them.
    ...
    ptThresh = 0.01;
    pointsA = detectFASTFeatures(imgA, 'MinContrast', ptThresh, 'MinQuality', ptThresh); % , 'ROI', [30 100 80 140]);
    pointsB = detectFASTFeatures(imgB, 'MinContrast', ptThresh, 'MinQuality', ptThresh); % , 'ROI', [30 100 80 140]);
...
function createVideoFromImagesSequence(working_dir, video_images_dir, images_format, video_name)
    % Converts the images sequence into a .avi video

    imageNames = dir(fullfile(working_dir, video_images_dir, images_format));
    imageNames = {imageNames.name}';

    outputVideo = VideoWriter(fullfile(working_dir, video_name));
    % outputVideo.FrameRate = shuttleVideo.FrameRate;
    open(outputVideo)

    for ii = 1:length(imageNames)
        img = imread(fullfile(working_dir, video_images_dir ,imageNames{ii}));
...
function H = cvexEstStabilizationTform2(leftI,rightI,ptThresh)
    ...
    pointsA = detectFASTFeatures(leftI, 'MinContrast', ptThresh, 'ROI', [30 100 80 100]);
    pointsB = detectFASTFeatures(rightI, 'MinContrast', ptThresh, 'ROI', [30 100 80 100]);
