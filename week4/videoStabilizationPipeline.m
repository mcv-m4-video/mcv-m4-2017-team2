...
function videoStabilizationPipeline
    ...
    [time, AUC, TP_, TN_, FP_, FN_, precision, recall, F1] = alpha_sweep(data, alpha_vect, mu_matrix, sigma_matrix, range_images, start_img, dirInputs, input_files, background, foreground, dirGT, rho_vect);
end


function [time, AUC, TP_, TN_, FP_, FN_, precision, recall, F1] = alpha_sweep(data, alpha_vect, mu_matrix, sigma_matrix, range_images, start_img, dirInputs, input_files, background, foreground, dirGT, rho_vect)
%function for sweeping through several thresholds to compare performance
    ...
    for n=1:size(alpha_vect,2)
        ...
        for rho = rho_vect
            ...
            for i=round(range_images/2)+2:range_images
                index = i + start_img -1;
