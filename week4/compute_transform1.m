function err = compute_transform1(I_old, I_curr, p)
    ...
    % Displacement:
    [r1prime, r2prime] = affine_transform2(r1, r2, p);
...
%     global_affine_visualize2(I_old, p, step)
    ...
    subr1prime = r1prime(r1low:r1high, r2low:r2high);
    subr2prime = r2prime(r1low:r1high, r2low:r2high);
    
    if(sum(sum(subr1prime<=0)) + ...
            sum(sum(subr1prime>nrow)) + ...
            sum(sum(subr2prime<=0)) + ...
            sum(sum(subr2prime>ncol)) > 0)
        sum(sum(subr1prime<=0)) + ...
            sum(sum(subr1prime>nrow)) + ...
            sum(sum(subr2prime<=0)) + ...
            sum(sum(subr2prime>ncol))
        disp('We are going out of the image...')
        err = Inf;
