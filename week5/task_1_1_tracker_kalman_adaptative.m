...
function task_1_1_tracker_kalman_adaptative()
...
if strcmp(sequence, 'nova_icaria')
    T1 = 1174;
    T2 = 2004;
    dirInputs = './sequence_parc_nova_icaria_320x240/';
end
...
%mu_matrix and sigma_matrix initial
obj.mu_matrix = mu_matrix;
obj.sigma_matrix = sigma_matrix;
...
for idx = 1:nframes
    ...
    frame = imread(strcat(dirInputs, filename));
    %train background
...
% remove noisy pixels and to fill the holes in the remaining blobs.
    ...
    function [centroids, bboxes, mask] = detectObjects(frame)
        ...
        % Detect foreground.
        alpha = 2.75;  % best value with traffic stabilized
        rho = 0.075; 
        mask = double(abs(frame - obj.mu_matrix) >= (alpha * (obj.sigma_matrix + 2)));
        ...
        roi = imread('mask_roi_parc_nova_icaria2_320x240.png');
        roi = double(roi(:,:,1) > 0.5);
        mask = logical(mask .* roi);
        
        
        [obj.mu_matrix, obj.sigma_matrix] = adaptModel(frame, mask, obj.mu_matrix, obj.sigma_matrix, rho);
