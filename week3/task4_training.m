...
function exhaustive_grid_search(start_img, range_images, dirInputs, input_files, dirGT, background, foreground, alpha_val, rho_val)
    ...
    beta1_vals = [0.4];  % 0.1:0.05:1;
    beta2_vals = [0.6];  % 0.1:0.05:1;
    ts_vals = [0.5];  % 0.1:0.05:1;
    th_vals = [0.1];  % 0.1:0.05:1;
        ...
        for beta2_val = beta2_vals
            for ts_val = ts_vals
                for th_val = th_vals
                    ...
                    fprintf(['\ntrying beta1 = ', num2str(beta1_val), ' and beta2 = ', num2str(beta2_val), ' and ts = ', num2str(ts_val), ' and th = ', num2str(th_val), '\n']);
...
function [precision, recall, F1] = single_alpha_adaptive(alpha_val, rho_val, mu_matrix, sigma_matrix, range_images, start_img, dirInputs, input_files, background, foreground, background_rgb, dirGT, create_animated_gif, beta1, beta2, ts, th)
    ...
    for i=1:(round(range_images/2)+1)
        ...
        % remove shadows
        [detection, shadows] = removeShadowsChromaticity(frame_rgb, detection_with_shadows, background_rgb, beta1, beta2, ts, th);
...
function [foreground, shadows] = removeShadowsChromaticity(frame, foreground, background, beta1, beta2, ts, th)
